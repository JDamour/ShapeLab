...
public static class Export {
    ...
    public static void exportSTLfromBuffer(Vert[] vertexData, string name)
    {
        //check if there is a Folder for Pottery in Documents
        //if not create a new Folder
        string path = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "/ShapeLab/";
        if (!Directory.Exists(path))
        {
            Directory.CreateDirectory(path);
        }
        ...
        {
            if (!(vertexData[i].position.x == vertexData[i].position.y && vertexData[i].position.x == vertexData[i].position.z && vertexData[i].position.y == vertexData[i].position.z))
            {
                // Write the normal of the triangle to the file
                streamWriter.WriteLine("  facet normal " + vertexData[i].normal.x + " " + vertexData[i].normal.y + " " + vertexData[i].normal.z);

                // Write the 3 vertices to the file
                streamWriter.WriteLine("    outer loop");
                streamWriter.WriteLine("      vertex " + vertexData[i].position.x + " " + vertexData[i].position.y + " " + vertexData[i].position.z);
                streamWriter.WriteLine("      vertex " + vertexData[i + 1].position.x + " " + vertexData[i + 1].position.y + " " + vertexData[i + 1].position.z);
                streamWriter.WriteLine("      vertex " + vertexData[i + 2].position.x + " " + vertexData[i + 2].position.y + " " + vertexData[i + 2].position.z);
                streamWriter.WriteLine("    endloop");
                streamWriter.WriteLine("  endfacet");
            }
        }
